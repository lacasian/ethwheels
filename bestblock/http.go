...
// runHTTP polls the node for the best block number every [b.config.PollInterval]
func (b *Tracker) runHTTP() {
	b.logger.Tracef("tracking best block via HTTP polling, every %s", b.config.PollInterval)
	for {
		select {
