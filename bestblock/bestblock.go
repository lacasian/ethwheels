...
func NewTracker(config Config) (*Tracker, error) {
	return &Tracker{
		...
		logger:      logrus.WithField("module", "eth"),
		config:      config,
	}, nil
}
...
func (b *Tracker) Run() {
	b.logger.Info("starting best block tracker")
	...
	for {
		...
		if b.config.WS != "" {
			b.logger.Info("setting up websocket connection")
			conn, err = ethrpc.NewWithDefaults(b.config.WS)
		} else {
			b.logger.Info("setting up best block polling")
			conn, err = ethrpc.NewWithDefaults(b.config.HTTP)
		}
		if err != nil {
			b.logger.Error(err)
...
func (b *Tracker) publish(block int64) {
	...
	b.logger.WithField("block", block).Tracef("will publish to %d clients", len(b.subscribers))
...
func (b *Tracker) Subscribe() chan int64 {
	...
	b.logger.Trace("new client subscribed")
...
func (b *Tracker) Unsubscribe(c chan int64) {
